param(
    ...
    [string]$AppName = "OIScreenerPortable"
)
...
$SourceRoot = Join-Path $RepoRoot "oi_screener"
$BuildRoot = Join-Path $RepoRoot ".build\windows-oi-screener"
...
foreach ($file in $SourceFiles) {
    Copy-Item -Path (Join-Path $SourceRoot $file) -Destination (Join-Path $AppDir $file) -Force
}
...
@echo off
set "APP_ROOT=%~dp0"
set "OI_SCREENER_HOME=%APP_ROOT%data"
set "PYTHONNOUSERSITE=1"
cd /d "%APP_ROOT%app"
echo Starting OI Screener on http://127.0.0.1:8503
start "" "http://127.0.0.1:8503"
"%APP_ROOT%python\python.exe" -m streamlit run app.py --server.address 127.0.0.1 --server.port 8503 --server.headless true --browser.gatherUsageStats false
pause
"@

Set-Content -Path (Join-Path $PortableRoot "Start OI Screener.bat") -Value $StartBat -Encoding ASCII

$Readme = @"
OI Screener Portable

Start:
1. Run "Start OI Screener.bat".
2. The app opens at http://127.0.0.1:8503.
3. Logs and state are stored in the local data folder.
...
Write-Host "Copy this folder to any Windows 11 PC and run Start OI Screener.bat"

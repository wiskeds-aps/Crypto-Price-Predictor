...
async function loadKlines() {
  ...
  try {
    const res = await fetch(`/api/futures/${chartSymbol}/klines?interval=${chartTf}&limit=400`);
    ...
    // Load all active indicators in parallel
    loadIndicators();
  } catch (e) {
...
async function loadIndicators() {
  await Promise.all([
    activeInds.has('oi')  ? loadOI()  : Promise.resolve(),
    activeInds.has('cvd') ? Promise.resolve(loadCVD()) : Promise.resolve(),
    activeInds.has('ls')  ? loadLS()  : Promise.resolve(),
  ]);
  // Зафитить на общий диапазон где есть все данные, затем синхронизировать
  _fitCommonRange();
}
...
async function loadOI() {
  ...
  _oiStartTime = null;
  const startTime = _klineData[0].time;
  try {
    const res = await fetch(
      `/api/futures/${chartSymbol}/oi?interval=${chartTf}&limit=500&start_time=${startTime}`
    );
    if (!res.ok) return;
...
async function loadLS() {
  ...
  try {
    const res = await fetch(
      `/api/futures/${chartSymbol}/ls-ratio?interval=${chartTf}&limit=500&start_time=${startTime}`
    );
    if (!res.ok) return;
    const data = await res.json();
...
async function loadFutures() {
  ...
  try {
    ...
    tbody.innerHTML = list.map((f, i) => {
      ...
      return `
      <tr class="${rowCls}" onclick="openChart(${JSON.stringify(f).replace(/"/g, '&quot;')})">

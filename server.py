...
WEB_ROOT = Path(os.environ.get("BTC_FORECAST_WEB_ROOT", Path(__file__).resolve().parent))
PROJECT_ROOT = Path(os.environ.get("TIMESFM_PROJECT_ROOT", "/root/timesfm-project"))
RIVER_CURRENT = Path(os.environ.get("RIVER_CURRENT_PATH", "/root/river/live/forecast_1h.json"))
RIVER_HISTORY = Path(os.environ.get("RIVER_HISTORY_PATH", "/root/river/live/history_1h.csv"))
CRYPTO_RIVER_CURRENT = Path(
    os.environ.get("CRYPTO_RIVER_CURRENT_PATH", WEB_ROOT / "crypto_river_24h.json")
)
CRYPTO_RIVER_HISTORY = Path(
    os.environ.get("CRYPTO_RIVER_HISTORY_PATH", WEB_ROOT / "crypto_river_24h_history.csv")
)
...
if __name__ == "__main__":
    host = os.environ.get("BTC_FORECAST_HOST", "127.0.0.1")
    port = int(os.environ.get("BTC_FORECAST_PORT", "8080"))
    server = ThreadingHTTPServer((host, port), Handler)
    print(f"Forecast site listening on {host}:{port}")
